...
\begin{algorithm}[H]
\caption{静态基准方案穷举搜索模型}
\label{alg:static_exhaustive}
\begin{algorithmic}[1]
\REQUIRE 患者个体数据 $P$ (包含：年龄组 $age\_grp$，活动量表总分 $adl\_total$，初始痰湿积分 $phlegm\_init$)
\REQUIRE 成本参数与约束条件 (详见附表2、附表3)
\ENSURE 全局最优干预方案 $(x_1^*, x_2^*, x_3^*)$ 及预期最终积分 $Score_{final}$

\STATE 根据 $phlegm\_init$ 查表确定适用调理等级集合 $X_1$
\STATE 初始化最优解：$best\_score \gets 100$，$best\_plan \gets \emptyset$

\FOR{$x_1 \in X_1$}
    \FOR{$x_2 \in \{1, 2, 3\}$}
        \FOR{$x_3 \in \{1, 2, \dots, 10\}$}
            \STATE // 约束检查
            \IF{不满足年龄约束$(age\_grp, x_2)$}
                \STATE \textbf{continue}
            \ENDIF
            \IF{不满足活动能力约束$(adl\_total, x_2)$}
                \STATE \textbf{continue}
            \ENDIF
            
            \STATE // 计算总成本
            \STATE $Cost \gets 6 \times Cost_{cond}(x_1) + 24 \times x_3 \times Cost_{act}(x_2)$
            \IF{$Cost > 2000$}
                \STATE \textbf{continue}
            \ENDIF
            ...
            \STATE // 计算疗效（痰湿积分下降率）
            \IF{$x_3 \ge 5$}
                \STATE $R \gets (x_2 - 1) \times 0.03 + (x_3 - 5) \times 0.01$
            \ELSE
                \STATE $R \gets 0$
            \ENDIF
            
            \STATE // 计算6个月后预期积分
            \STATE $Score \gets phlegm\_init \times (1 - R)^6$
            
            \STATE // 更新最优解（积分越低越好）
            \IF{$Score < best\_score$}
                \STATE $best\_score \gets Score$
                \STATE $best\_plan \gets (x_1, x_2, x_3)$
            \ENDIF
        \ENDFOR
    \ENDFOR
\ENDFOR

\RETURN $best\_plan, best\_score$
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
\caption{多阶段动态优化模型（以 $K$ 个月为一个阶段）}
\label{alg:dynamic_optimization}
\begin{algorithmic}[1]
\REQUIRE 患者个体数据 $P$，阶段时长 $T$ (月)，总周期 $T_{total}=6$ 月
\ENSURE 各阶段最优方案序列及总成本

\STATE 初始化当前积分 $phlegm\_curr \gets phlegm\_init$
\STATE 初始化阶段列表 $Stages \gets []$
\STATE $t \gets 0$

\WHILE{$t < T_{total}$}
    \STATE // 根据当前积分确定当前阶段适用的调理等级
    \STATE $X_1\_curr \gets$ 根据 $phlegm\_curr$ 查表确定
    
    \STATE // 调用静态模型（算法 \ref{alg:static_exhaustive}）求解本阶段最优方案
    \STATE $(x_1, x_2, x_3) \gets$ 穷举搜索(当前积分$phlegm\_curr$，剩余预算)
    
    \STATE // 计算本阶段结束后的新积分
    \STATE $R \gets (x_2 - 1) \times 0.03 + (x_3 - 5) \times 0.01$ (若$x_3 \ge 5$，否则为0)
    \STATE $phlegm\_next \gets phlegm\_curr \times (1 - R)^T$
    
    \STATE // 记录本阶段信息
    \STATE $Stages.append(\{t, t+T, x_1, x_2, x_3, phlegm\_curr, phlegm\_next\})$
    
    \STATE // 更新状态
    \STATE $phlegm\_curr \gets phlegm\_next$
    \STATE $t \gets t + T$
\ENDWHILE

\STATE 计算总成本 $Cost\_total \gets \sum_{stage \in Stages} (T \times Cost_{cond}(x_1) + 4T \times x_3 \times Cost_{act}(x_2))$

\RETURN $Stages, Cost\_total, phlegm\_curr$
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
\caption{基于综合效用的多目标权衡模型}
\label{alg:utility_optimization}
\begin{algorithmic}[1]
\REQUIRE 患者个体数据 $P$
\REQUIRE 偏好权重 $\omega_1, \omega_2$ (满足 $\omega_1+\omega_2=1$)
\REQUIRE 最大成本 $Cost_{max} = 2000$
\ENSURE 综合效用最优的方案及对应的效用值

\STATE 根据 $phlegm\_init$ 确定 $X_1$
\STATE 初始化最优效用 $best\_Z \gets \infty$，$best\_plan \gets \emptyset$

\FOR{$x_1 \in X_1$}
    \FOR{$x_2 \in \{1, 2, 3\}$}
        \FOR{$x_3 \in \{1, 2, \dots, 10\}$}
            \STATE // 约束检查（同算法 \ref{alg:static_exhaustive}）
            \IF{不满足年龄或活动能力约束}
                \STATE \textbf{continue}
            \ENDIF
            ...
            \ELSE
                \STATE $R \gets 0$
            \ENDIF
            \STATE $\Delta Phlegm \gets phlegm\_init \times (1 - (1 - R)^6)$
            
            \STATE // 离差标准化（归一化）
            \STATE $\hat{C} \gets Cost / Cost_{max}$
            \STATE $\hat{B} \gets \Delta Phlegm / phlegm\_init$
            
            \STATE // 计算综合效用函数值（最小化目标）
            \STATE $Z \gets \omega_1 \times \hat{C} - \omega_2 \times \hat{B}$
            ...
            \IF{$Z < best\_Z$}
                ...
                \STATE $best\_Score \gets phlegm\_init \times (1 - R)^6$
            \ENDIF
        \ENDFOR
    \ENDFOR
\ENDFOR

\RETURN $best\_plan, best\_Z, best\_Cost, best\_Score$
\end{algorithmic}
\end{algorithm}
...
% 四个层标题
\node[title] (in) {输入层};
\node[title, right=of in] (con) {约束检查层};
\node[title, right=of con] (opt) {优化引擎层};
\node[title, right=of opt] (out) {决策输出层};

% 输入层条目
\node[box, below=0.1cm of in] (in1) {人口学特征};
\node[box, below=0cm of in1] (in2) {体质积分};
\node[box, below=0cm of in2] (in3) {活动评分};
\node[box, below=0cm of in3] (in4) {偏好权重};

% 约束检查层条目
\node[box, below=0.1cm of con] (con1) {年龄约束};
\node[box, below=0cm of con1] (con2) {活动能力约束};
\node[box, below=0cm of con2] (con3) {预算约束};
\node[box, below=0cm of con3] (con4) {耐受度约束};

% 优化引擎层条目
\node[box, below=0.1cm of opt] (opt1) {静态穷举搜索};
\node[box, below=0cm of opt1] (opt2) {动态阶段优化};
\node[box, below=0cm of opt2] (opt3) {综合效用权衡};
\node[box, below=0cm of opt3] (opt4) {灵敏度分析};

% 决策输出层条目
\node[box, below=0.1cm of out] (out1) {最优方案推荐};
\node[box, below=0cm of out1] (out2) {预期积分轨迹};
\node[box, below=0cm of out2] (out3) {成本-效益分析};
\node[box, below=0cm of out3] (out4) {匹配规律表};

% 层间箭头
\draw[arrow] (in.east) -- (con.west);
\draw[arrow] (con.east) -- (opt.west);
\draw[arrow] (opt.east) -- (out.west);

\end{tikzpicture}
\caption{模型完备形态的四层架构}
\label{fig:model_architecture_tikz}
\end{figure}

\begin{algorithm}[H]
\caption{完备形态模型：融合动态调整与综合效用的干预方案优化}
\label{alg:integrated_model}
\begin{algorithmic}[1]
\REQUIRE 患者个体数据 $P$ (年龄组 $age\_grp$，活动量表总分 $adl\_total$，初始痰湿积分 $phlegm\_init$)
\REQUIRE 阶段时长 $T \gets 2$ (月)，总干预周期 $T_{total} \gets 6$ (月)
\REQUIRE 最大总预算 $Budget_{max} \gets 2000$ (元)
\REQUIRE 偏好权重系数 $\omega_1 \gets 0.5$， $\omega_2 \gets 0.5$ (成本与疗效同等重要)
\ENSURE 各阶段最优方案序列 $Stages$，总成本 $Cost\_total$，最终痰湿积分 $phlegm\_final$，综合效用值 $Z\_final$

\STATE // 初始化状态变量
\STATE $phlegm\_curr \gets phlegm\_init$
\STATE $Budget\_remain \gets Budget_{max}$
\STATE $t \gets 0$
\STATE $Stages \gets \emptyset$
\STATE $Cost\_total \gets 0$
\STATE $Benefit\_total \gets 0$

\WHILE{$t < T_{total}$}
    \STATE // 根据当前积分确定当前阶段适用的调理等级集合
    \STATE $X_1\_curr \gets$ 根据 $phlegm\_curr$ 查表 (附表2) 确定的可行调理等级
    \STATE // 计算本阶段剩余预算约束 (按比例分配)
    \STATE $Budget\_stage\_max \gets Budget\_remain \times \frac{T}{T_{total} - t}$
    
    \STATE // 调用静态综合效用优化内核 (偏好权重 $\omega_1=\omega_2=0.5$)
    \STATE $best\_Z \gets \infty$
    \STATE $best\_plan \gets \emptyset$
    \FOR{$x_1 \in X_1\_curr$}
        \FOR{$x_2 \in \{1, 2, 3\}$}
            \FOR{$x_3 \in \{1, 2, \dots, 10\}$}
                \IF{不满足年龄约束$(age\_grp, x_2)$ 或 不满足活动能力约束$(adl\_total, x_2)$}
                    \STATE \textbf{continue}
                \ENDIF
                \STATE $Cost\_stage \gets T \times Cost_{cond}(x_1) + 4T \times x_3 \times Cost_{act}(x_2)$
                \IF{$Cost\_stage > Budget\_stage\_max$}
                    \STATE \textbf{continue}
                \ENDIF
                \STATE // 计算本阶段疗效
                \IF{$x_3 \ge 5$}
                ...
                \ELSE
                    \STATE $R \gets 0$
                \ENDIF
                \STATE $\Delta Phlegm \gets phlegm\_curr \times (1 - (1 - R)^T)$
                \STATE // 计算归一化成本与收益
                \STATE $\hat{C} \gets Cost\_stage / Budget\_stage\_max$
                \STATE $\hat{B} \gets \Delta Phlegm / phlegm\_curr$
                \STATE $Z \gets 0.5 \times \hat{C} - 0.5 \times \hat{B}$  \COMMENT{偏好权重固定为0.5}
                \IF{$Z < best\_Z$}
                    \STATE $best\_Z \gets Z$
                    \STATE $best\_plan \gets (x_1, x_2, x_3)$
                    \STATE $best\_Cost \gets Cost\_stage$
                    \STATE $best\_Delta \gets \Delta Phlegm$
                \ENDIF
            \ENDFOR
        \ENDFOR
    \ENDFOR
    ...
    \STATE // 执行本阶段最优方案
    \STATE $(x_1^*, x_2^*, x_3^*) \gets best\_plan$
    \STATE $R^* \gets$ 根据 $x_2^*, x_3^*$ 计算的月下降率
    \STATE $phlegm\_next \gets phlegm\_curr \times (1 - R^*)^T$
    
    \STATE // 记录本阶段信息
    \STATE $Stages \gets Stages \cup \{(t, t+T, x_1^*, x_2^*, x_3^*, phlegm\_curr, phlegm\_next, best\_Cost)\}$
    
    \STATE // 更新全局状态
    \STATE $Cost\_total \gets Cost\_total + best\_Cost$
    \STATE $Benefit\_total \gets Benefit\_total + best\_Delta$
    \STATE $Budget\_remain \gets Budget\_remain - best\_Cost$
    \STATE $phlegm\_curr \gets phlegm\_next$
    \STATE $t \gets t + T$
\ENDWHILE

\STATE // 计算最终综合效用值 (基于总成本和总收益)
\STATE $\hat{C}_{final} \gets Cost\_total / Budget_{max}$
\STATE $\hat{B}_{final} \gets Benefit\_total / phlegm\_init$
\STATE $Z_{final} \gets 0.5 \times \hat{C}_{final} - 0.5 \times \hat{B}_{final}$

\RETURN $Stages, Cost\_total, phlegm\_curr, Z_{final}$
\end{algorithmic}
\end{algorithm}
